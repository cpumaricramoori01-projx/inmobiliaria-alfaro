...
const iniciales: Visita[] = [
  { id: "INM-0061", posicion: "61", nombre: "Casa Urbanización Buenos Aires", ubicacion: "Chimbote", tipo: "Casa", propietario: "María López", dni: "45871236" },
  { id: "INM-0064", posicion: "64", nombre: "Terreno Los Álamos", ubicacion: "Nuevo Chimbote", tipo: "Terreno", propietario: "José Ramírez", dni: "40987654" },
];
...
export default function Page() {
  ...
  const marcarRealizada = (item: Visita) => {
    setError(null);
    if (!fechas[item.id]) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {mensaje && (
          <div className="mt-6 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100">✓</span>
            <div>
              {mensaje}
              <p className="mt-1 text-xs font-normal text-emerald-700">Se registra usuario, fecha y observaciones en la trazabilidad del inmueble.</p>
            </div>
          </div>
        )}
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-violet-200 bg-violet-50/70 p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-violet-700/80">transición automática del flujo</p>
          </div>
          <div className="rounded-2xl border border-cyan-200 bg-cyan-50/70 p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-cyan-700">Registrado por</p>
            <p className="mt-2 text-lg font-bold text-cyan-950">{usuarioActual}</p>
            <p className="mt-1 text-xs text-cyan-700/80">se reemplazará por el usuario autenticado</p>
          </div>
        </div>
        ...
        <section className="mt-6">
          <div className="mb-3 flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">Fecha y usuario quedan como parte de la trazabilidad automática.</p>
            </div>
          ...
          {items.length > 0 ? (
                  ...
                  <div className="p-5 lg:p-6">
                    <div className="flex flex-col gap-5">
                      ...
                      <div className="grid gap-4 lg:grid-cols-2">
                        <div>
                          <label htmlFor={`fecha-${item.id}`} className="text-xs font-semibold text-slate-600">Fecha de visita realizada *</label>
                          <input
                            id={`fecha-${item.id}`}
                            type="date"
                            max={hoy}
                            value={fechas[item.id] ?? ""}
                            onChange={(e) => setFechas((actual) => ({ ...actual, [item.id]: e.target.value }))}
                        ...
                        <div>
                          <label htmlFor={`drive-${item.id}`} className="text-xs font-semibold text-slate-600">Evidencia / fotografías en Google Drive (opcional)</label>
                          <input
                            id={`drive-${item.id}`}
                            type="url"
                            value={enlacesDrive[item.id] ?? ""}
                            onChange={(e) => setEnlacesDrive((actual) => ({ ...actual, [item.id]: e.target.value }))}
                            placeholder="https://drive.google.com/..."
                            className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 outline-none transition focus:border-violet-300 focus:bg-white focus:ring-2 focus:ring-violet-100"
                          />
                          <p className="mt-1 text-[11px] text-slate-400">Por ahora se guarda el enlace; el archivo no se almacena en la aplicación.</p>
                        </div>
                      </div>

                      <div>
                        <label htmlFor={`obs-${item.id}`} className="text-xs font-semibold text-slate-600">Observación de la visita (opcional)</label>
                        <textarea
                          id={`obs-${item.id}`}
                          rows={3}
                          value={observaciones[item.id] ?? ""}
                          onChange={(e) => setObservaciones((actual) => ({ ...actual, [item.id]: e.target.value }))}
                      ...
                      <div className="flex flex-col gap-3 rounded-xl bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">Se completará la visita y el inmueble pasará a <strong>Tasación pendiente</strong>.</p>
                        </div>
                        <button
                          type="button"
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 px-6 py-12 text-center">
              ...
              <p className="mt-1 text-sm text-emerald-800/80">Los inmuebles completados continúan en la etapa de tasación pendiente.</p>
        ...
        <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">Trazabilidad y archivos</p>
