    ...
    <main className="min-h-screen bg-[#f7f7f5] px-5 py-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex items-center gap-2"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#fdf2f2] text-[#c80000]">✓</span><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-400">Fase 1 · Registro de actividad</p></div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white border border-slate-200 p-5 text-white shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Acción principal</p><p className="mt-1 text-sm font-semibold">Confirma la fecha real de la visita y deja la evidencia o notas.</p></div><span className="rounded-full border border-violet-300/20 bg-violet-300/10 px-3 py-1.5 text-xs font-bold text-violet-200">Siguiente: Tasación</span></div></section>

        {mensaje && <div className="mt-6 rounded-xl border border-[#ead8d8] bg-[#fdf2f2] px-4 py-3 text-sm font-semibold text-slate-800">{mensaje}</div>}
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf7f7]/70 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-[#c80000]">Al completar</p><p className="mt-2 text-lg font-bold text-slate-950">Tasación pendiente</p><p className="mt-1 text-xs text-[#c80000]/80">transición guardada en BD</p></div>
          <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf7f7]/70 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-[#c80000]">Trazabilidad</p><p className="mt-2 text-lg font-bold text-slate-950">Automática</p><p className="mt-1 text-xs text-[#c80000]/80">usuario, fecha y evento</p></div>
        </div>

        <section className="mt-6">
          <div className="mb-3 flex items-end justify-between"><div><h2 className="font-bold text-slate-900">Visitas por confirmar</h2><p className="mt-1 text-xs text-slate-500">La lista proviene directamente de los inmuebles en visita_pendiente.</p></div><span className="rounded-full bg-[#fdf7f7] px-3 py-1 text-xs font-bold text-[#c80000]">{items.length} pendientes</span></div>
          {cargando ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Cargando visitas pendientes...</div> : items.length > 0 ? (
                  ...
                  <div className="p-5 lg:p-6">
                    <div className="flex flex-col gap-5">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="flex items-center gap-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#fdf7f7] text-base font-bold text-[#c80000]">{item.posicion}</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-slate-900">{item.nombre}</h3><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">{item.tipo}</span></div><p className="mt-1 text-xs text-slate-500">{item.ubicacion} · {item.codigo}</p><p className="mt-1 text-xs text-slate-500">Propietario: {item.propietario} · DNI {item.dni}</p></div></div>
                        <span className="self-start rounded-full bg-[#fdf7f7] px-3 py-1.5 text-xs font-bold text-[#c80000]">Visita pendiente</span>
                      </div>
            ...
            <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf2f2]/70 px-6 py-12 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-xl text-[#c80000]">✓</div><h3 className="mt-4 font-bold text-slate-950">No hay visitas pendientes</h3><p className="mt-1 text-sm text-slate-800/80">La bandeja está sincronizada con la base de datos.</p><Link href="/tasaciones-textos-pendientes" className="mt-5 inline-flex rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">Ver siguiente etapa</Link></div>
          )}
        </section>
      </div>
    </main>
