    ...
    <main className="min-h-screen bg-[#f7f7f5] px-5 py-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <div className="mt-2 flex items-start gap-3">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#fdf2f2] text-[#c80000]">ₛ</span>
              <div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white border border-slate-200 p-5 text-white shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold">Registra la tasación y deja definida la situación frente al propietario.</p>
            </div>
        ...
        <section className="mt-7 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf7f7]/70 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-[#c80000]">Tasación vigente</p><p className="mt-2 text-3xl font-bold text-slate-950">{aprobadas.length}</p><p className="mt-1 text-xs text-[#c80000]/80">aprobadas actualmente</p></div>
          <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf7f7]/70 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-[#c80000]">Fuente</p><p className="mt-2 text-lg font-bold text-slate-950">Base de datos</p><p className="mt-1 text-xs text-[#c80000]/80">sin datos ficticios</p></div>
        </section>
        ...
        <section className="mt-7">
          ...
          {cargando ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Cargando...</div> : items.length === 0 ? (
            <div className="rounded-2xl border border-[#ead8d8] bg-[#fdf2f2]/70 px-6 py-12 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-xl text-[#c80000]">✓</div><h3 className="mt-4 font-bold text-slate-950">No hay tasaciones pendientes</h3><p className="mt-1 text-sm text-slate-800/80">Las visitas realizadas ya tienen tasación registrada o no existen inmuebles pendientes.</p></div>
          ) : (
                  ...
                  <div className="p-5 lg:p-6">
                    <div className="flex flex-col gap-5">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <span className="self-start rounded-full bg-[#fdf7f7] px-3 py-1.5 text-xs font-bold text-[#c80000]">Tasación pendiente</span>
                      </div>
                      ...
                      <div className="flex flex-col gap-3 rounded-xl bg-[#f7f7f5] p-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-slate-500">Al guardar, el inmueble avanza automáticamente según la situación registrada.</p><button disabled={guardando === item.inmuebleId} onClick={() => registrar(item)} className="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{guardando === item.inmuebleId ? "Guardando..." : "Registrar tasación"}</button></div>
                    </div>
                  </div>
                </article>
              ))}
