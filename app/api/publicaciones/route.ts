...
async function published(){return db.select({inmuebleId:inmInmuebles.id,codigo:inmInmuebles.codigo,tipo:inmInmuebles.tipo,referencia:inmInmuebles.referencia,posicion:inmPosiciones.numero,publicado:inmPublicaciones.publicado,fechaPublicacion:inmPublicaciones.fechaPublicacion}).from(inmPublicaciones).innerJoin(inmInmuebles,eq(inmInmuebles.id,inmPublicaciones.inmuebleId)).leftJoin(inmAsignacionesPosicion,and(eq(inmAsignacionesPosicion.inmuebleId,inmInmuebles.id),eq(inmAsignacionesPosicion.activa,true))).leftJoin(inmPosiciones,eq(inmPosiciones.id,inmAsignacionesPosicion.posicionId)).where(and(eq(inmInmuebles.estado,"activo"),eq(inmPublicaciones.publicado,true))).orderBy(desc(inmPublicaciones.fechaPublicacion));}
export async function GET(){try{const rows=await approved();const publicadas=await published();return NextResponse.json({ok:true,pendientes:rows.map(map),publicadas});}catch(error){console.error(error);return NextResponse.json({ok:false,error:"No se pudieron consultar los textos pendientes."},{status:500});}}
