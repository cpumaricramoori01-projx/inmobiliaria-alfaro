  ...
  return (
    <main className="min-h-screen bg-[#f7f7f5] px-5 py-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white border border-slate-200 p-4 text-white shadow-sm">
        ...
        <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className={`rounded-full px-3 py-1.5 text-[10px] font-bold ${puedeRegistrar ? "bg-[#fdf2f2] text-[#c80000]" : "bg-slate-100 text-slate-500"}`}>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            {[
              ...
              <div key={String(num)} className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${done ? "border-[#ead8d8] bg-[#fdf2f2]/70" : "border-slate-200 bg-slate-50/60"}`}>
                <span className={`flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold ${done ? "bg-[#c80000] text-white" : "bg-white text-slate-400"}`}>{done ? "✓" : num}</span>
                <div><p className={`text-xs font-semibold ${done ? "text-slate-800" : "text-slate-600"}`}>{label}</p><p className="text-[10px] text-slate-400">{done ? "Completo" : "Pendiente"}</p></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_320px]">
          <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 bg-slate-50/70 px-6 py-5">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#fdf2f2] text-xl font-bold text-[#c80000]">+</span>
            ...
            <div className="space-y-6 p-6">
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-[#fdf2f2] px-2.5 py-1 text-[10px] font-bold text-[#c80000]">{disponibles.length} disponibles</span>
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
                ...
                {propietarioEncontrado && <div className="mt-4 rounded-xl border border-[#ead8d8] bg-[#fdf2f2] px-4 py-3"><p className="text-xs font-bold text-[#c80000]">Propietario encontrado</p><p className="mt-1 text-xs text-[#c80000]">Se reutilizará el propietario registrado y se actualizarán sus datos de contacto.</p></div>}
              ...
              {mensaje && <div className="rounded-xl border border-[#ead8d8] bg-[#fdf2f2] p-4"><p className="text-sm font-semibold text-[#c80000]">Registro realizado</p><p className="mt-1 text-xs leading-5 text-[#c80000]">{mensaje}</p></div>}

              <div className="rounded-xl bg-[#fdf7f7] p-4"><p className="text-sm font-semibold text-slate-800">Después del registro</p><p className="mt-1 text-xs leading-5 text-[#c80000]">El inmueble quedará automáticamente en <strong>Visita pendiente</strong>, la posición quedará ocupada y se generará el primer evento de historial.</p></div>
              ...
              <div className="flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
                ...
                <button onClick={registrar} disabled={!puedeRegistrar} className="rounded-xl bg-[#c80000] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-200">{guardando ? "Registrando..." : "Registrar inmueble"}</button>
              </div>
            </div>
          </section>
          ...
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              <div className="flex gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#fdf2f2] text-sm font-bold text-[#c80000]">1</span><div><p className="text-sm font-semibold text-slate-800">Registrar inmueble</p><p className="mt-0.5 text-xs text-slate-500">Posición + tipo + referencia + propietario.</p></div></div>
              <div className="ml-4 h-5 border-l border-dashed border-slate-200"/>
              <div className="flex gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#fdf7f7] text-sm font-bold text-[#c80000]">2</span><div><p className="text-sm font-semibold text-slate-800">Visita pendiente</p><p className="mt-0.5 text-xs text-slate-500">Aparece automáticamente en la bandeja.</p></div></div>
              <div className="ml-4 h-5 border-l border-dashed border-slate-200"/>
              <div className="flex gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#fdf7f7] text-sm font-bold text-[#c80000]">3</span><div><p className="text-sm font-semibold text-slate-800">Tasación</p><p className="mt-0.5 text-xs text-slate-500">Se habilita al completar la visita.</p></div></div>
            </div>
            <div className="mt-5 rounded-xl bg-[#f7f7f5] p-4"><p className="text-xs font-semibold text-slate-700">Capacidad</p><p className="mt-1 text-lg font-bold text-slate-950">90 <span className="text-sm font-medium text-slate-400">posiciones administradas</span></p><p className="mt-2 text-[11px] text-slate-400">Disponibles: <strong>{cargandoPosiciones ? "..." : disponibles.length}</strong> · La disponibilidad se consulta directamente en la base de datos.</p></div>
          </aside>
        </div>
      </div>
    </main>
